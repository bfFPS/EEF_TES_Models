...
min_max_scenarios = zeros(length(unique_stores)*numb_weeks, 4);
...
for iStore = 1:length(unique(daily_scenarios_table.Store_ID))
    ...
    for iWeek = 1:numb_weeks
        ...
        min_max_scenarios(iWeek + (iStore-1)*numb_weeks, 1) = high_scenario;
...
%         min_max_scenarios(iWeek + (iStore-1)*numb_weeks, 4) = low_scenario_orders;
        min_max_scenarios(iWeek + (iStore-1)*numb_weeks, 3) = this_store;
        min_max_scenarios(iWeek + (iStore-1)*numb_weeks, 4) = high_scenario_orders;
...
min_max_scenarios_output = array2table(min_max_scenarios,...
    'VariableNames', daily_scenarios_table.Properties.VariableNames(1:4));

min_max_scenarios_output_joined = innerjoin(min_max_scenarios_output, daily_scenarios_table);

min_max_scenarios_output_joined = sortrows(min_max_scenarios_output_joined, 'Week');
min_max_scenarios_output_joined = sortrows(min_max_scenarios_output_joined, 'Store_ID');
