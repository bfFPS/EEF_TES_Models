...
user_path = 'C:\Users\FPSScripting2\Documents\Consolidated_Models\OrderModelling\NewPostcodeRegression';
...
PCS_store_allocations = readtable(fullfile(user_path, 'Inputs\catchment_areas_old_new.xlsx'));

% % read cleaned order data with post code segments included
order_data_PCS = readtable(fullfile(user_path, 'Inputs\uniform_PC_order_data_ALL.csv'));
order_data_PCS_all = order_data;
order_data_PCS_all.PCS = order_data_PCS_all.Post_Code;

order_data_PCS_all.PCS_test = order_data_PCS_all.PCS;
order_data_PCS_all.PCS_test = cellfun(@(S) S(1:end-2), order_data_PCS_all.PCS_test, 'Uniform', 0);

this_year = 2019;
order_data_PCS = order_data_PCS_all(year(order_data_PCS_all.Order_Delivery_Date) == this_year, :);

% read in the output of the data processing script
processed_order_data = readtable(fullfile(user_path, 'Inputs\historic_weekly_orders_MPO_HPO_by_PCS.csv'));

% find the unique weeks in the data
weeks_this_year = unique(week(order_data_PCS_all.Order_Delivery_Date));
...
%     this_week_data = order_data_PCS(ceil(day(order_data_PCS.Order_Delivery_Date, 'dayofyear')./days_in_week) == this_week,:);
    this_week_data = order_data_PCS_all(week(order_data_PCS_all.Order_Delivery_Date) == this_week,:);
...
proportion_MPO_HPO_by_PCS.HistoricOrders = processed_order_data_required_PCS.Orders;

% save the output tables to the output folder for this Step 1
writetable(proportion_MPO_HPO_by_PCS, fullfile(user_path, 'Outputs', strcat(num2str(this_year), '_PCS_stats.xlsx')), 'Sheet', 'Proportion_HPO_MPO_by_PCS');
writetable(annual_weekly_volumes, fullfile(user_path, 'Outputs', strcat(num2str(this_year), '_PCS_stats.xlsx')), 'Sheet', 'Annual_Weekly_Volumes');
writetable(PCS_store_allocations, fullfile(user_path, 'Outputs', strcat(num2str(this_year), '_PCS_stats.xlsx')), 'Sheet', 'PCS_Store_Allocations');
