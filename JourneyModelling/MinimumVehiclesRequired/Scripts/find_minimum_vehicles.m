...
user_path = 'C:\Users\FPSScripting2\Documents\Consolidated_Models\JourneyModelling\MinimumVehiclesRequired';
...
data = readtable(fullfile(user_path, 'Inputs\20200317VehicleReducer2.xlsx'), 'Sheet', 'Weekly Journeys');
% future vehicle numbers forecast by data provider
forecast_vehicle_numbers = readtable(fullfile(user_path, 'Inputs\future_vehicles.xlsx'));
% non-vehicle parameters by year and store
non_vehicle_parameters = readtable(fullfile(user_path, 'Inputs\input_non_vehicle_parameters_vehicle_reducer.xlsx'));
% daily ratios by year and store
ratios_matrix = readtable(fullfile(user_path, 'Inputs\input_ratios_vehicle_reducer.xlsx'));
% vehicle parameters and charger ratings
vehicle_parameters = readtable(fullfile(user_path, 'Inputs\202001VehicleAssumptions_MATLAB_formatV3.xlsx'));
...
% save output table to output folder 
writetable(output_table, fullfile(user_path, 'Outputs', strcat(run_date, '.', client, '.', 'output_vehicle_numbers', '.', run_version, '.', user_initials, '.csv')));
